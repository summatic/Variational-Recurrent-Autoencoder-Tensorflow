...
def read_data(token_ids_path, config, max_size=None):
    """Read data from source and target files and put into buckets.

    Args:
      token_ids_path: path to the files with token-ids for the source language.
    ...
    with tf.gfile.GFile(token_ids_path, mode="r") as file:
...
def create_model(session, config, forward_only):
    ...
    weight_initializer = tf.orthogonal_initializer if config.orthogonal_initializer \
        else tf.uniform_unit_scaling_initializer
...
def train(config):
    ...
    train_ids, dev_ids, _ = data_utils.prepare_data(config.data_dir, config.vocab_size)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        while True:
            ...
            step_loss_summaries.append(tf.Summary(
                value=[tf.Summary.Value(tag="step loss",
                                        simple_value=float(step_loss))]))
            step_KL_loss_summaries.append(tf.Summary(
                value=[tf.Summary.Value(tag="KL step loss",
            ...
            if current_step % config.steps_per_checkpoint == 0:
                ...
                for bucket_id in xrange(len(config.buckets)):
                    ...
                    eval_perp_summary = tf.Summary(
                        value=[tf.Summary.Value(tag="eval perplexity for bucket {0}".format(bucket_id),
                ...
                eval_loss_summary = tf.Summary(
                    value=[tf.Summary.Value(tag="mean eval loss",
...
def decode(sess, model, config, means, logvars, bucket_id):
    fr_vocab_path = os.path.join(config.data_dir,
                                 "vocab%d" % config.vocab_size)
    _, rev_vocab = data_utils.initialize_vocabulary(fr_vocab_path)
...
def main(_):
    ...
    configs["model"]["data_dir"] = FLAGS.model_dir + 'corpus'
